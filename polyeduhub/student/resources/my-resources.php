...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_resource') {
    ...
    $check_stmt = $pdo->prepare("SELECT id FROM resources WHERE id = ? AND user_id = ?");
    $check_stmt->execute([$resource_id, $user_id]);
    
    if ($check_stmt->rowCount() > 0) {
        // Delete the resource
        $delete_stmt = $pdo->prepare("DELETE FROM resources WHERE id = ?");
        $delete_stmt->execute([$resource_id]);
        ...
        $log_stmt->execute([
            $user_id, 
            "Resource Deleted", 
            "Deleted resource with ID: " . $resource_id
        ]);
        
        // Subtract points for deletion
        $points_stmt = $pdo->prepare("INSERT INTO points_history (user_id, points, action, description) VALUES (?, ?, ?, ?)");
        $points_stmt->execute([
            $user_id, 
            -5, 
            "Resource Deletion", 
            "Points deducted for deleting a resource"
        ]);
        
        // Update total points
        $update_points_stmt = $pdo->prepare("UPDATE user_points SET points = points - 5 WHERE user_id = ?");
        $update_points_stmt->execute([$user_id]);
        
        $_SESSION['success_message'] = "Resource deleted successfully.";
        header("Location: my-resources.php");
        exit();
    } else {
        $_SESSION['error_message'] = "You are not authorized to delete this resource.";
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (confirm(`Are you sure you want to delete the resource "${resourceTitle}"? This action cannot be undone.`)) {
