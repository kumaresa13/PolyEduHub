...
try {
    ...
    try {
        ...
        $stats['uploaded_resources'] = $stmt->fetchColumn() ?: 0;
        
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM resource_downloads WHERE user_id = ?");
        $stmt->execute([$user_id]);
        $stats['downloaded_resources'] = $stmt->fetchColumn() ?: 0;
        ...
        // Get user's rank
        $stmt = $pdo->prepare("
            SELECT COUNT(*) + 1 FROM user_points WHERE points > (
                SELECT points FROM user_points WHERE user_id = ?
            )
        ");
        $stmt->execute([$user_id]);
        $stats['rank'] = $stmt->fetchColumn() ?: 'N/A';
    } catch (PDOException $e) {
        // Silently fail and use default values if queries fail
    }

    // Recent activities placeholder data
    $recent_activities = [];
    
    // Try to get real activities if database is connected
    try {
        $stmt = $pdo->prepare("
            SELECT 'upload' as type, r.title, r.created_at as timestamp, 'fa-file-upload' as icon
            FROM resources r
            WHERE r.user_id = ?
            UNION
            SELECT 'download' as type, r.title, rd.downloaded_at as timestamp, 'fa-file-download' as icon
            FROM resource_downloads rd
            JOIN resources r ON rd.resource_id = r.id
            WHERE rd.user_id = ?
            UNION
            SELECT 'comment' as type, r.title, rc.created_at as timestamp, 'fa-comment' as icon
        ...
        // Silently fail and use empty array if query fails
    }

    // Recommended resources placeholder data
    $recommended_resources = [];
    
    // Try to get real recommended resources if database is connected
    try {
        $stmt = $pdo->prepare("
            SELECT r.id, r.title, rc.name as category, u.first_name || ' ' || u.last_name as uploaded_by,
                   COALESCE(AVG(rr.rating), 0) as rating, r.download_count as downloads
            FROM resources r
    ...
    // Database connection error, use placeholder data
    $pdo = null;
    
    // Placeholder data for when database is unavailable
    $stats = [
        'uploaded_resources' => 0,
        'downloaded_resources' => 0,
        'points_earned' => 0,
